...
class FlyDSLAOTCompilerTest(unittest.TestCase):
    ...
    def test_export_uses_packed_entry_and_explicit_module_symbols(self):
        ...
        with (
            ...
            mock.patch(
                "torch._inductor.codegen.flydsl.aot_compile.ExecutionEngine",
                _FakeExecutionEngine,
            ),
            mock.patch(
                "torch._inductor.codegen.flydsl.aot_compile._resolve_runtime_libs",
    ...
    def test_runtime_library_publication_is_atomic(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(1, sum(isinstance(error, RuntimeError) for error in errors))
            self.assertIn(destination.read_bytes(), (first.read_bytes(), second.read_bytes()))
    ...
    def test_tensor_abi_describes_dynamic_layout(self):
        tensor = torch.empty_strided((2, 3), (5, 1))
    ...
    def test_abi_rejects_unsupported_and_multi_slot_arguments(self):
        ...
        with (
            mock.patch(
                "torch._inductor.codegen.flydsl.aot_compile.c_abi_spec",
                return_value=[(ctypes.c_int32, mock.Mock()), (ctypes.c_int32, mock.Mock())],
            ),
    ...
    def test_compile_aot_does_not_dispatch_launcher(self):
        ...
        with (
            ...
            mock.patch(
                "torch._inductor.codegen.flydsl.aot_compile.get_backend",
                return_value=backend,
            ),
            mock.patch(
                "torch._inductor.codegen.flydsl.aot_compile.MlirCompiler.compile",
