...
class TestCodegenTriton(InductorTestCase):
    ...
    def test_auxiliary_write_region(self):
        ...
        write = ir.AuxiliaryWriteRegion(
            ...
            predicate=sympy.And(
                auxiliary_index >= 512, auxiliary_index < 768
            ),
            value=127,
        )
        or_write = ir.AuxiliaryWriteRegion(
            ...
            predicate=sympy.Or(
                auxiliary_index < 128, auxiliary_index >= 896
            ),
            value=0,
        )
    ...
    def test_auxiliary_write_dependencies(self):
        ...
        self.assertIn(
            dependencies.StarDep("buf0"), buffer.get_read_writes().writes
        )
        symbol_uses = buffer.get_free_symbol_uses(unbacked_only=True)
        self.assertTrue(
            {auxiliary_numel, auxiliary_offset, auxiliary_limit}.issubset(
                symbol_uses
            )
        )
