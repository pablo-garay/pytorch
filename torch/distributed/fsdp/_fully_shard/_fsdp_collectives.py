...
def foreach_reduce(
    ...
    with device_handle.stream(post_reduce_stream):
        ...
        uniform_grad_dtype = (
            next(iter(explicit_grad_dtypes)) if len(explicit_grad_dtypes) == 1 else None
        )
        ...
        for padded_unsharded_size, fsdp_param in zip(
            ...
            if uniform_grad_dtype is None:
                # Non-uniform group: the shared cast above could not pick one
