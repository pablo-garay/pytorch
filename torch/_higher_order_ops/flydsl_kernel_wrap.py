...
class FlyDSLLauncherSideTable:
    def __init__(self) -> None:
        ...
        self.launcher_to_id: dict[tuple[int, int | None, frozenset[str] | None], int] = {}
    ...
    def add_launcher(self, registration: FlyDSLLauncherRegistration) -> int:
        key = (
            ...
            id(registration.bound_self) if registration.bound_self is not None else None,
...
def _is_compile_time_parameter(parameter: inspect.Parameter) -> bool:
    from flydsl.compiler.jit_argument import is_type_param_annotation
...
def _register_flydsl_call_spec(
    ...
    key = tuple(
        (
            idx,
            ("constexpr", Constexpr.value_signature(value))
...
def _validate_runtime_arguments(
    ...
    from flydsl.compiler.protocol import JitArgument
    from flydsl.expr.typing import Stream

    for parameter, value in zip(registration.signature.parameters.values(), args):
        if _is_compile_time_parameter(parameter):
            continue
        ...
        if isinstance(value, JitArgument):
            raise TypeError(
    ...
    if registration.mutates_args is None:
        return
    for parameter, value in zip(registration.signature.parameters.values(), args):
        if parameter.name in registration.mutates_args and not isinstance(value, Tensor):
            raise TypeError(
...
def partition_flydsl_launcher_arguments(
    ...
    for idx, (parameter, value) in enumerate(zip(parameters, args)):
        annotation = parameter.annotation
        ...
        elif annotation is not inspect.Parameter.empty and is_type_param_annotation(
            annotation
        ):
            constant_args[idx] = value
            runtime_args[idx] = None
...
def split_flydsl_launcher_arguments(
    ...
    for parameter, value in zip(parameters, args):
        if parameter.kind in (
            ...
            raise TypeError("FlyDSL launchers with variadic parameters cannot be captured")
...
def flydsl_kernel_wrapper_mutation_functionalize(
    ...
    mutated_arg_indices = tuple(
        get_mutated_arg_indices(launcher_idx, unwrapped_args)
    )
...
def _validate_clone_aliases(
    ...
    for position, (arg_idx, arg) in enumerate(tensor_args):
        for other_idx, other in tensor_args[position + 1 :]:
            ...
            ) and torch._C._is_alias_of(arg, other):
                raise RuntimeError(
