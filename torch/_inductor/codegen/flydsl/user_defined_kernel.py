...
class UserDefinedFlyDSLKernel(ir.ExternKernel):
    ...
    def __init__(
        ...
        *,
        ...
        if not inputs:
            raise AssertionError("FlyDSL launcher must have at least one tensor argument")
...
def _mutated_arg_indices(launcher_idx: int, args: tuple[Any, ...]) -> tuple[int, ...]:
    ...
    if registration.mutates_args is None:
        return tuple(idx for idx, arg in enumerate(args) if isinstance(arg, ir.TensorBox))
