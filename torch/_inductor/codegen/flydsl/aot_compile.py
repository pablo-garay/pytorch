...
from flydsl.compiler.protocol import c_abi_spec, construct_from_ir_values, get_ir_types
from flydsl.expr.meta import tracing_context
from flydsl.expr.numeric import BFloat16, Float16, Float32, Float64, Numeric
from flydsl.expr.typing import Constexpr, Stream
from flydsl.expr.utils.arith import fastmath as fastmath_ctx
...
def _numeric_ctype_name(value: Numeric, ctype: type) -> str:
    if value.signed is not None:
        if value.width == 1:
            return "bool"
        return f"{'int' if value.signed else 'uint'}{value.width}"
    if isinstance(value, (Float16, BFloat16)):
        return "uint16"
...
def _argument_abi(value: Any) -> list[dict[str, Any]]:
    if isinstance(value, MemRefJitArg):
        slots = [
            {
                "kind": "tensor_data",
...
def _launcher_abi(sig, bound, jit_args: list[Any], has_user_stream: bool) -> tuple[dict[str, Any], ...]:
    abi = []
    jit_arg_index = 0
...
def _rename_export_symbols(module: ir.Module, entry: str, symbol: str) -> None:
    symbol_map = {}
...
class CompiledAOTLauncher:
    def __init__(self, module: ir.Module, entry: str, abi: tuple[dict[str, Any], ...]):
    ...
    def export_to_c(self, object_file_path: str, function_name: str) -> dict[str, Any]:
        ...
        if not object_path.parent.exists():
            raise FileNotFoundError(f"object output directory does not exist: {object_path.parent}")

        runtime_libraries = _resolve_runtime_libs()
...
def compile_aot(launcher: JitFunction, *args, **kwargs) -> CompiledAOTLauncher:
    ...
    hints = (
        CompilationContext.compile_hints(launcher.compile_hints)
        if launcher.compile_hints
        else nullcontext()
    )
    with _create_mlir_context() as ctx, hints:
        ...
        loc = func_def_location(launcher.func, ctx)
        ...
        with ir.InsertionPoint(module.body), loc:
            ...
            with CompilationContext.create() as comp_ctx:
                ...
                with ir.InsertionPoint(entry_block):
                    ...
                    fastmath_flag = effective_fastmath_hint(
                        CompilationContext.get_compile_hints()
                    )
                    fastmath_scope = (
                        fastmath_ctx(fastmath_flag)
                        if fastmath_flag is not None
                        else nullcontext()
                    )
                    with tracing_context(launcher.func), fastmath_scope:
        ...
        if link_libs and _use_external_binary_codegen():
            raise RuntimeError(
        ...
        log().info(f"FlyDSL AOT jit_args={jit_args}")
