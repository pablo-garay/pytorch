...
def padded_xdl_scale_scatter(
    ...
    required_physical_rows = (
        FloorDiv(rows + logical_row_chunk - 1, logical_row_chunk)
        * physical_row_chunk
    )
    sizevars = V.graph.sizevars
    ...
    sizevars.check(
        sympy.Eq(Mod(padded_rows, physical_row_chunk), sympy.S.Zero)
    )
    sizevars.check(
        sympy.Eq(Mod(padded_rows, 2 * xdl), sympy.S.Zero)
    )
    ...
    def pointwise_fallback():
        ...
        def inner_fn(index):
            physical_row_in_chunk, logical_row, logical_col = logical_indices(
                index[0]
            )
            valid = ops.and_(
                ops.index_expr(
    ...
    if not all(
        ...
        if not (
            ...
            and V.graph.has_feature(
                device, BackendFeature.AUXILIARY_WRITE_REGIONS
            )
        ...
        def shuffled_offset(physical_row, col):
            ...
            return (
                (((
                    (
                        row_outer * FloorDiv(padded_cols, col_chunk)
                        + col_outer
                    )
                    * col_inner
